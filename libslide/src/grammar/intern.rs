    ...
    ($($intern_macro:ident, $ty:ty, $interned_struct:ident, $rodeo:ident, $spur2expr:ident)*) => {$(
        ...
        impl $interned_struct {
            ...
            pub(crate) fn intern(expr: $ty) -> Self {
                ...
                let mb = $rodeo.read().unwrap().get(hash);
                match mb {
                    Some(spur) => Self(spur),
                    None => {
                        let spur = $rodeo.write().unwrap().get_or_intern(expr.emit_s_expression());
                        $spur2expr.write().unwrap().insert(spur, expr);
                        Self(spur)
                    }
                }
            }
        ...
        impl Deref for $interned_struct {
            ...
            fn deref(&self) -> &<Self as InternedExpression>::Inner {
                let table = $spur2expr.read().unwrap();
                let val = table.get(&self.0).unwrap();
                // Safety:
